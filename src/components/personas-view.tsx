...
import { Plus, Eye, Pencil, Trash2, Loader2, Users } from "lucide-react";
...
const limpioOguion = (v: string | null) => (v && v !== "-" ? v : null);

const COLUMNAS: Column<Persona>[] = [
  {
    key: "nombre",
    header: "Nombre",
    accessor: (p) => p.nombre,
    render: (p) => p.nombre || <span className="text-muted-foreground">—</span>,
  ...
  },
  {
    key: "tipo_persona",
    header: "Tipo",
    accessor: (p) => labelDe(TIPO_PERSONA, p.tipo_persona),
    render: (p) => <Badge variant="outline">{labelDe(TIPO_PERSONA, p.tipo_persona)}</Badge>,
  },
  {
    key: "ind_cliente_proveedor",
    header: "Cli/Prov",
    accessor: (p) => labelDe(IND_CLI_PROV, p.ind_cliente_proveedor),
  },
  {
    key: "nro_ci",
    header: "CI",
    accessor: (p) => limpioOguion(p.nro_ci),
    className: "tabular-nums",
  },
  {
    key: "nro_ruc",
    header: "RUC",
    accessor: (p) => limpioOguion(p.nro_ruc),
    className: "tabular-nums",
  },
  {
    key: "nro_telefono",
    header: "Teléfono",
    accessor: (p) => limpioOguion(p.nro_telefono),
  },
];
      ...
      <div className="p-4 sm:p-5">
        {isLoading ? (
          ...
          <div className="grid place-items-center py-16 text-center">
            ...
            <p className="mt-4 font-medium">Aún no hay personas</p>
